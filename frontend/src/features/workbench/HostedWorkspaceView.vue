...
import { AgentIdeVueWorkbench } from "@b10x/agentide-ui/vue";
...
<template>
  ...
  <AgentIdeVueWorkbench
    v-else
    :bottom-open="terminalOpen"
    :bottom-height="terminalHeight"
    explorer-label="Project explorer and context pins"
    inspector-label="Session inspector"
    center-label="Hosted coding workspace"
    bottom-label="Hosted terminal panel"
  >
    <template #titlebar>
    ...
    <template #explorer>
      ...
      <div v-if="treeTruncated" class="tree-boundary-state">
        ...
        {{
          treeOmitted == null ? "Additional count is unknown." : `${treeOmitted} entries omitted.`
        }}
      </div>
      ...
      <section class="explorer-pins">
        ...
        <button v-if="selectedRange" class="button small" type="button" @click="attachSelection">
    ...
    <template #center>
      <nav class="workbench-pane-tabs" aria-label="Workspace panes">
        ...
        <button type="button" :class="{ active: activePane === 'diff' }" @click="setPane('diff')">
          <FileDiff :size="14" /> Diff
        </button>
        <button type="button" :class="{ active: activePane === 'agent' }" @click="setPane('agent')">
          <Bot :size="14" /> Agent
        </button>
      </nav>
    ...
    <template #inspector>
      ...
      <section v-if="rightPane === 'context'">
        ...
        <article v-for="attachment in attachments" :key="attachment.id" class="context-attachment">
          <strong>{{ attachment.label }}</strong>
      ...
      <section v-else-if="rightPane === 'agents'">
        ...
        <p v-if="!workspace.agents.length" class="muted">No agents are available to this actor.</p>
      </section>
      <section v-else-if="rightPane === 'grants'">
        ...
        <article v-for="grant in agentIdeGrants" :key="grant.grant_id" class="authority-card">
          ...
          <small>
            {{ grant.maximum_risk }} risk · paths {{ grant.path_prefixes.join(", ") }} · revision
            {{ grant.revision }}
          </small>
          <button
    ...
    <template #bottom>
      ...
      <header>
        <button type="button" @click="terminalOpen = !terminalOpen">
          <PanelBottomClose v-if="terminalOpen" :size="15" /><PanelBottomOpen v-else :size="15" />
          Terminal
        </button>
        <span
          v-if="terminalOpen"
      ...
      <div v-if="terminalOpen" class="terminal-panel">
        ...
        <div v-else class="terminal-ready-panel">
          <div class="terminal-profile-actions">
            <label>
              ...
              <select v-model="selectedTerminalProfileId">
                <option v-for="profile in terminalProfiles" :key="profile.id" :value="profile.id">
                  {{ profile.label }} · {{ profile.workspace_access.replace("_", " ") }} · network
                  {{ profile.network }}
                </option>
              </select>
            </label>
          ...
          <nav v-if="visibleTerminals.length" class="terminal-tabs" aria-label="Terminal sessions">
            <article
          ...
          <template v-if="activeTerminal">
            <div class="terminal-session-metadata">
              ...
              <span><strong>Process</strong> {{ activeTerminal.process_id ?? "allocating" }}</span>
