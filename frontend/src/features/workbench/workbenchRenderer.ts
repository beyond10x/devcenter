...
import type { WorkbenchController } from "@b10x/agentide-ui/controller";
...
export function mountWorkbenchRenderer(
  ...
  const renderer = controller.mount(
    {
      ...target,
      mount(element, options) {
        return target.mount(element, {
          ...options,
          dispatch(action) {
            if (action.kind === "refresh") void refresh();
            else options.dispatch(action);
          },
        });
      },
    },
  ...
  return {
    ...
    editor: {
      ...
      create(element, options) {
        ...
        if (!options.readOnly) markWorkspaceStage("editable-file");
        return {
